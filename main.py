...
class AIDataset(Dataset):
    def __init__(self, root, train, transform=None):
        ...
        ########## Revise the dataset directory ##########
        if train:
            self.data_dir = "./input/hw2-dataset/train/";
            self.file_list = os.listdir(self.data_dir)
            self.file_list.sort()
        else:
            self.data_dir = "./input/hw2-dataset/test/";
            self.file_list = os.listdir(self.data_dir)
            self.file_list.sort()
        ##################################################
    ...
    def Preprocessing(self, img):
        ...
        # titles = ['Original', 'Normalization', 'Thresholding']
        # images = [img, img1, img2]
        # for i in range(3):
        #     plt.subplot(
        #         1, 3, i+1), plt.imshow(images[i], cmap='gray', vmin=0, vmax=255)
        #     plt.title(titles[i])
...
def Load():
    train_dataset = AIDataset(root='', train=True, transform=transforms.ToTensor())
    test_dataset = AIDataset(root='', train=False, transform=transforms.ToTensor())
